...
def enrich_data(batch, batch_id):
    results = []
    stock = convert_arr_to_pd_stock(batch)
    for i in range(0, len(batch) - 1):
        row = batch[i]
        # from https://pypi.org/project/stockstats/
        if i < 20 or i >= len(batch) - 20:
            result = {'Value': row['Value'], 'Date': row['Date'],
                      'Will_Raise': False,
                      'Month': row['Date'][0:2], 'Batch_ID': batch_id}
        else:
            result = {'Will_Raise': check_rise(row['Value'], batch, i + 1), 'Value': row['Value'],
                      'Date': row['Date'], 'Month': row['Date'][0:2], 'Batch_ID': batch_id,
                      'cr': stock['cr'].iloc[i],
...
def transform_to_mid(json_data):
    ...
    for row in json_data:
        ...
        if (current_time - entry_time).seconds >= BATCH_TIME_FRAME_SECONDS:
            if len(batch) > 40:
                result += enrich_data(batch, batch_id)
